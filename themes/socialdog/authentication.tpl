	...
	<div class="row">
		...
		<div class="col-sm-12 logincontainer">
			<form action="{$link->getPageLink('authentication', true)|escape:'html':'UTF-8'}" method="post" id="login_form" >
				<h3 class="page-subheading">{l s='Already registered? Sign in to your account'}</h3>
				
				<div class="form-group clearfix">
					<input class="is_required validate account_input form-control" data-validate="isEmail" type="email" id="email" name="email" value="{if isset($smarty.post.email)}{$smarty.post.email|stripslashes}{/if}" placeholder="{l s='Email'}" />
...
{else}
	...
	<form action="{$link->getPageLink('authentication', true)|escape:'html':'UTF-8'}" method="post" id="account-creation_form" class="std box">
		{$HOOK_CREATE_ACCOUNT_TOP} <!-- @PODI - actually this hook is empty -->
		<div class="account_creation">
			<h3 class="page-subheading">{l s='Registration'}</h3> 
			...
			<div class="col-xs-4">
				...
				<div class="required form-group">
					...
					<br />
					{foreach from=$genders key=k item=gender}
						<div class="radio-inline"><label>{$gender->name}</label>
							<label for="id_gender{$gender->id}" class="top">
							<input type="radio" name="id_gender" id="id_gender{$gender->id}" value="{$gender->id}" {if isset($smarty.post.id_gender) && $smarty.post.id_gender == $gender->id}checked="checked"{/if} />
						</div>
					{/foreach}
			...
			<div class="col-xs-4">
				...
				{foreach from=$dlv_all_fields item=field_name}
					{if $field_name eq "company"}
						{if !$b2b_enable}
							<p class="form-group">
								<label for="company">{l s='Name of dog station'}{if in_array($field_name, $required_fields)} <sup>*</sup>{/if}</label>
								<input type="text" class="form-control" id="company" name="company" value="{if isset($smarty.post.company)}{$smarty.post.company}{/if}" />
							</p>
						{/if}
					{elseif $field_name eq "vat_number"}
			...
			<!-- 3. column - registration data form -->
			<div class="col-xs-4">	
				{if isset($newsletter) && $newsletter}
					<div class="checkbox">
