...
<html{if isset($language_code) && $language_code} lang="{$language_code|escape:'html':'UTF-8'}"{/if}>
	...
	{if !isset($content_only) || !$content_only}
		...
		<div id="{if ($page_name eq 'index') or ($page_name eq 'authentication')}{'page_intro'}{else}{'page_global'}{/if}">
			...
			{if $page_name eq 'my-account'}
				{$HOOK_CUSTOMER_ACCOUNT_HEADER}
			{/if}
			<div class="columns-container {if $page_name eq 'category'}{if ($category->level_depth eq '3')}{'catdesc'}{/if}{/if}">
				<div id="columns" class="container"> <!-- {if ($page_name eq 'index') or ($page_name eq 'authentication')}{''}{else}{' bg-white'}{/if} -->
					...
					*}
					...
					<div class="row">
						...
						<div id="center_column" class="center_column col-xs-12 col-sm-{$cols|intval}{if $page_name eq 'category'}{if isset($category) && ($category->level_depth eq '3')}{' catdesc'}{/if}{/if}">
